...
describe('Util', () => {
  describe('#getContextValue', () => {
    it('should return the fallback when the context does not contain the given key', async() => {
      expect(Util.getContextValue(new JsonLdContextNormalized({}), 'a', 'x', 'FB')).toBe('FB');
    });

    it('should return the fallback when the context contains the given key, without contextKey', async() => {
      expect(Util.getContextValue(new JsonLdContextNormalized({ x: {}}), 'a', 'x', 'FB')).toBe('FB');
    });

    it('should return the value when the context contains the given key, with contextKey', async() => {
      expect(Util.getContextValue(new JsonLdContextNormalized({ x: { a: 'b' }}), 'a', 'x', 'FB')).toBe('b');
    });
  });
  ...
  describe('#getContextValueType', () => {
    it('should return null as default', async() => {
      expect(Util.getContextValueType(new JsonLdContextNormalized({}), 'abc')).toBeNull();
    });
  ...
  describe('#getContextValueLanguage', () => {
    it('should return null as default', async() => {
      expect(Util.getContextValueLanguage(new JsonLdContextNormalized({}), 'abc')).toBeNull();
    });
    ...
    it('should return the null entry language even if a root @language is present', async() => {
      expect(Util.getContextValueLanguage(
        new JsonLdContextNormalized({ abc: { '@language': null }, '@language': 'nl-be' }),
        'abc',
      ))
        .toBeNull();
    });
  ...
  describe('#getContextValueDirection', () => {
    it('should return null as default', async() => {
      expect(Util.getContextValueDirection(new JsonLdContextNormalized({}), 'abc')).toBeNull();
    });
    ...
    it('should return the null entry direction even if a root @direction is present', async() => {
      expect(Util.getContextValueDirection(
        new JsonLdContextNormalized({ abc: { '@direction': null }, '@direction': 'ltr' }),
        'abc',
      ))
        .toBeNull();
    });
  ...
  describe('#getContextValueIndex', () => {
    it('should return null as default', async() => {
      expect(Util.getContextValueIndex(new JsonLdContextNormalized({}), 'abc')).toBeNull();
    });
  ...
  describe('#isPropertyReverse', () => {
    ...
    it('should return true when the key has @reverse in the context', async() => {
      expect(Util.isPropertyReverse(
        new JsonLdContextNormalized({ abc: { '@reverse': 'bla' }}),
        'abc',
        'def',
      )).toBe(true);
    });
  ...
  describe('instance', () => {
    ...
    describe('#valueToTerm', () => {
      ...
      describe('for an object', () => {
        it('without an @id should return a bnode', async() => {
          await expect(util.valueToTerm(context, 'key', {}, 0, [])).resolves
        ...
        it('without an @id should put a blank node on the id stack when a value was emitted at a deeper depth', async() => {
          ...
          expect(util.parsingContext.idStack[1]).toEqualRdfTermArray([ DF.blankNode() ]);
        });

        it('with an @id should return a named node', async() => {
          await expect(util.valueToTerm(context, 'key', { '@id': 'http://ex.org' }, 0, [])).resolves
            .toEqualRdfTermArray([ DF.namedNode('http://ex.org') ]);
        });

        it('with a relative @id without @base in context should return []', async() => {
          await expect(util.valueToTerm(context, 'key', { '@id': 'abc' }, 0, [])).resolves
            .toEqual([]);
        });
        ...
        it('with a relative to parent with query @id with @base in context should return a named node', async() => {
          context = new JsonLdContextNormalized({ '@base': 'http://ex.org/abc/' });
          await expect(util.valueToTerm(context, 'key', { '@id': '..?a=b' }, 0, [])).resolves
        ...
        it('with an empty @id with baseIRI should return a named node', async() => {
          ...
          await expect(util.valueToTerm(await util.parsingContext.getContext([]), 'key', { '@id': '' }, 0, [])).resolves
            .toEqualRdfTermArray([ DF.namedNode('http://ex.org/') ]);
        });
        ...
        it('with an empty @id with baseIRI and vocabIRI should return a named node for @type = @vocab', async() => {
          ...
          await expect(util.valueToTerm(context, 'key', { '@id': '', '@type': '@vocab' }, 0, [])).resolves
        ...
        it('with a relative @id and empty local @context with @base in parent context', async() => {
          context = new JsonLdContextNormalized({ '@base': 'http://ex.org/' });
          util.parsingContext.contextTree.setContext([], Promise.resolve(new JsonLdContextNormalized({ '@base': 'http://ex.org/' })));
          await expect(util.valueToTerm(context, 'key', { '@context': {}, '@id': 'abc' }, 0, [])).resolves
            .toEqualRdfTermArray([ DF.namedNode('http://ex.org/abc') ]);
        });

        it('with a relative @id and @base in local @context', async() => {
          const value = { '@context': { '@base': 'http://ex.org/' }, '@id': 'abc' };
          await expect(util.valueToTerm(new JsonLdContextNormalized({}), 'key', value, 0, [])).resolves
            .toEqualRdfTermArray([ DF.namedNode('http://ex.org/abc') ]);
        });

        it('with a relative @id and null local @context with @base in parent context', async() => {
          context = new JsonLdContextNormalized({ '@base': 'http://ex.org/' });
          await expect(util.valueToTerm(context, 'key', { '@context': null, '@id': 'abc' }, 0, [])).resolves
            .toEqual([]);
        });
        ...
        it('with a relative @id and no other properties with @base in parent context should return', async() => {
          ...
          util.parsingContext.contextTree.setContext([ 'key' ], Promise.resolve(new JsonLdContextNormalized({ '@base': 'http://ex.ignored.org/' })));
          await expect(util.valueToTerm(context, 'key', { '@id': 'abc' }, 0, [ 'key' ])).resolves
        ...
        it('with a relative @id and other properties with @base in inner context should return', async() => {
          ...
          util.parsingContext.contextTree.setContext([ 'key' ], Promise.resolve(new JsonLdContextNormalized({ '@base': 'http://ex.do.org/' })));
          await expect(util.valueToTerm(context, 'key', { '@id': 'abc', a: 'b' }, 0, [ 'key' ])).resolves
            .toEqualRdfTermArray([ DF.namedNode('http://ex.do.org/abc') ]);
        });
        ...
        it('with a relative @id and @context with @base in inner context should return', async() => {
          ...
          util.parsingContext.contextTree.setContext([ 'key' ], Promise.resolve(new JsonLdContextNormalized({ '@base': 'http://ex.ignored2.org/' })));
          const context2 = { '@base': 'http://ex.do.org/' };
          await expect(util.valueToTerm(context, 'key', { '@id': 'abc', '@context': context2 }, 0, [ 'key' ])).resolves
            .toEqualRdfTermArray([ DF.namedNode('http://ex.do.org/abc') ]);
        });

        it('with an @value should return a literal', async() => {
          await expect(util.valueToTerm(context, 'key', { '@value': 'abc' }, 0, [])).resolves
        ...
        it('with an @value and @language should return a lowercased language-tagged string literal in 1.0', async() => {
          util.parsingContext.activeProcessingMode = 1;
          await expect(util.valueToTerm(context, 'key', { '@value': 'abc', '@language': 'en-US' }, 0, [])).resolves
            .toEqualRdfTermArray([ DF.literal('abc', 'en-us') ]);
        });
        ...
        it('with an @value and @language should return a lowercased language-tagged string literal' +
          'if normalizeLanguageTags', async() => {
        ...
        it('with an @value and @type should return a typed literal', async() => {
          await expect(util.valueToTerm(context, 'key', { '@value': 'abc', '@type': 'http://type.com' }, 0, [])).resolves
        ...
        it('with a @value value and @language in the context entry should return a language literal', async() => {
          ...
          await expect(util.valueToTerm(context, 'key', { '@value': 'abc', '@language': 'nl-nl' }, 0, [])).resolves
            .toEqualRdfTermArray([ DF.literal('abc', 'nl-nl') ]);
        });

        it('with a @value null should return []', async() => {
          context = new JsonLdContextNormalized({ key: { '@language': 'en-us' }, '@language': 'nl-be' });
          await expect(util.valueToTerm(context, 'key', { '@value': null }, 0, [])).resolves
            .toEqual([]);
        });
        ...
        it('with a @value and boolean @language should throw an error', async() => {
          await expect(util.valueToTerm(context, 'key', { '@value': 'abc', '@language': true }, 0, []))
        ...
        it('with a @value and invalid @language should return []', async() => {
          await expect(util.valueToTerm(context, 'key', { '@value': 'abc', '@language': 'en us' }, 0, []))
            .resolves.toEqual([]);
        });
        ...
        it('with a @value and non-string @direction should throw', async() => {
          await expect(util.valueToTerm(context, 'key', { '@value': 'abc', '@direction': 3 }, 0, []))
        ...
        it('with a non-string @value and valid @direction should throw', async() => {
          await expect(util.valueToTerm(context, 'key', { '@value': 3, '@direction': 'rtl' }, 0, []))
        ...
        it('with a @value and invalid @direction should return []', async() => {
          await expect(util.valueToTerm(context, 'key', { '@value': 'abc', '@direction': 'r tl' }, 0, []))
            .resolves.toEqual([]);
        });
        ...
        it('with a @value and valid @direction rtl should return a plain literal', async() => {
          await expect(util.valueToTerm(context, 'key', { '@value': 'abc', '@direction': 'rtl' }, 0, []))
        ...
        it('with a @value and valid @direction ltr should return a plain literal', async() => {
          await expect(util.valueToTerm(context, 'key', { '@value': 'abc', '@direction': 'ltr' }, 0, []))
        ...
        it('with a @value and invalid string @type should throw an error', async() => {
          await expect(util.valueToTerm(context, 'key', { '@value': 'abc', '@type': 'a b' }, 0, []))
            .rejects.toThrow(new ErrorCoded('Invalid \'@type\' value, got \'"a b"\'', ERROR_CODES.INVALID_TYPED_VALUE));
        });

        it('with a @value and boolean @index should not throw an error', async() => {
          await expect(util.valueToTerm(context, 'key', { '@value': 'abc', '@index': true }, 0, [])).resolves
            .toEqualRdfTermArray([ DF.literal('abc') ]);
        });
        ...
        it('with conflicting @index values should not throw an error', async() => {
          ...
          await expect(util.valueToTerm(context, 'key', value, 0, [])).resolves
            .toEqual([]);
        });
        ...
        it('with a @value and ignoredkey should not throw an error', async() => {
          await expect(util.valueToTerm(context, 'key', { '@value': 'abc', ignoredkey: 'abc' }, 0, [])).resolves
            .toEqual([ DF.literal('abc') ]);
        });

        it('with a @value, @language and @type should throw an error', async() => {
          await expect(util.valueToTerm(context, 'key', { '@value': 'abc', '@language': 'en', '@type': 'abc' }, 0, []))
            .rejects.toThrow(new ErrorCoded('Can not have both \'@language\' and \'@type\' in a value: ' +
              '\'{"@value":"abc","@language":"en","@type":"abc"}\'', ERROR_CODES.INVALID_VALUE_OBJECT));
        });
        ...
        it('with a @value and blank node @type', async() => {
          await expect(util.valueToTerm(context, 'key', { '@value': 'abc', '@type': '_:b' }, 0, []))
            .rejects.toThrow(new ErrorCoded('Illegal value type (BlankNode): _:b', ERROR_CODES.INVALID_TYPED_VALUE));
        });

        it('with context-based @json type', async() => {
          context = new JsonLdContextNormalized({ key: { '@type': '@json' }});
          await expect(util.valueToTerm(context, 'key', { '@value': 'abc' }, 0, []))
            .resolves.toEqualRdfTermArray([ DF.literal('{"@value":"abc"}', DF.namedNode('http://www.w3.org/1999/02/22-rdf-syntax-ns#JSON')) ]);
        });

        it('with @value-based @json type', async() => {
          await expect(util.valueToTerm(context, 'key', { '@value': { v: 'abc' }, '@type': '@json' }, 0, []))
            .resolves.toEqualRdfTermArray([ DF.literal('{"v":"abc"}', DF.namedNode('http://www.w3.org/1999/02/22-rdf-syntax-ns#JSON')) ]);
        });
        ...
        it('with @value-based and context-based @json type', async() => {
          context = new JsonLdContextNormalized({ key: { '@type': '@json' }});
          await expect(util.valueToTerm(context, 'key', { '@value': { v: 'abc' }, '@type': '@json' }, 0, []))
            .resolves.toEqualRdfTermArray([ DF.literal('{"@type":"@json","@value":{"v":"abc"}}', DF.namedNode('http://www.w3.org/1999/02/22-rdf-syntax-ns#JSON')) ]);
        });
      });
      ...
      describe('for a string', () => {
        ...
        it('with an @type: @id should return a named node', async() => {
          ...
          await expect(util.valueToTerm(context, 'key', 'http://ex.org/', 0, [])).resolves
            .toEqualRdfTermArray([ DF.namedNode('http://ex.org/') ]);
        });
        ...
        describe('for language tags', () => {
          ...
          it('with a raw value and @direction+@language in the root context for rdfDirection i18n-datatype ' +
            'should return a language literal', async() => {
            util.parsingContext.rdfDirection = 'i18n-datatype';
            context = new JsonLdContextNormalized({ '@direction': 'rtl', '@language': 'en-us' });
            await expect(util.valueToTerm(context, 'key', 'abc', 0, [])).resolves
              .toEqualRdfTermArray([ DF.literal('abc', DF.namedNode('https://www.w3.org/ns/i18n#en-us_rtl')) ]);
          ...
          it('with a raw value and null @direction in the context entry should return a plain literal', async() => {
      ...
      describe('for a number', () => {
        ...
        it('for Infinity should return a INF', async() => {
          await expect(util.valueToTerm(context, 'key', Number.POSITIVE_INFINITY, 0, [])).resolves
            .toEqualRdfTermArray([ DF.literal('INF', DF.namedNode(Util.XSD_DOUBLE)) ]);
        });

        it('for -Infinity should return a -INF', async() => {
          await expect(util.valueToTerm(context, 'key', Number.NEGATIVE_INFINITY, 0, [])).resolves
            .toEqualRdfTermArray([ DF.literal('-INF', DF.namedNode(Util.XSD_DOUBLE)) ]);
        });

        it('for 1e20 should return a 1.0E20 as an integer', async() => {
          await expect(util.valueToTerm(context, 'key', 1e20, 0, [])).resolves
        ...
        it('for 1 with double context type should return 1.0E0', async() => {
          ...
          await expect(util.valueToTerm(context, 'key', 1, 0, [])).resolves
        ...
        it('for 1.1 with int context type should return 1.1E0', async() => {
          ...
          await expect(util.valueToTerm(context, 'key', 1.1, 0, [])).resolves
        ...
        it('for 100.1 with int context type should return 1.001E2', async() => {
          await expect(util.valueToTerm(context, 'key', 100.1, 0, [])).resolves
        ...
        it('for 123.45 with int context type should return 1.001E2', async() => {
          await expect(util.valueToTerm(context, 'key', 123.45, 0, [])).resolves
      ...
      describe('for an array', () => {
        it('should return []', async() => {
          await expect(util.valueToTerm(context, 'key', [ 1, 2 ], 0, [])).resolves.toEqual([]);
        });
      });

      describe('for a list', () => {
        it('should return []', async() => {
          await expect(util.valueToTerm(context, 'key', { '@list': [ 1, 2 ]}, 0, [])).resolves.toEqual([]);
        });

        it('should return rdf:nil for an empty anonymous list', async() => {
          await expect(util.valueToTerm(context, 'key', { '@list': []}, 0, [])).resolves
        ...
        it('should return null for a list with null elements', async() => {
          context = new JsonLdContextNormalized({ key: { '@container': { '@list': true }}});
          await expect(util.valueToTerm(context, 'key', [ null, null ], 0, [])).resolves.toEqual([]);
        });
      ...
      describe('for a set', () => {
        it('should return []', async() => {
          await expect(util.valueToTerm(context, 'key', { '@set': [ 1, 2 ]}, 0, [])).resolves.toEqual([]);
        });
      ...
      describe('for a reverse properties', () => {
        it('should return []', async() => {
          await expect(util.valueToTerm(context, 'key', { '@reverse': {}}, 0, [])).resolves.toEqual([]);
        });
      });
    });
    ...
    describe('#getPropertiesDepth', () => {
      it('should return depth for a string parent', async() => {
        await expect(util.getPropertiesDepth([ null, 'a', 'b' ], 2)).resolves.toBe(2);
      });
      ...
      it('should return depth - 1 for a @nest parent within an array', async() => {
        await expect(util.getPropertiesDepth([ null, 'a', '@nest', 0, 'b' ], 4)).resolves.toBe(2);
      });

      it('should return depth - 1 for a @nest parent within arrays', async() => {
        await expect(util.getPropertiesDepth([ null, 'a', '@nest', 0, 1, 2, 'b' ], 6)).resolves.toBe(2);
      });

      it('should return depth - 1 for a @nest parent within an array above @nest', async() => {
        await expect(util.getPropertiesDepth([ null, 'a', 0, '@nest', 'b' ], 4)).resolves.toBe(3);
      });

      it('should return depth - 1 for multiple @nest parents', async() => {
        await expect(util.getPropertiesDepth([ null, 'a', '@nest', '@nest', '@nest', 'b' ], 5)).resolves.toBe(2);
      });
