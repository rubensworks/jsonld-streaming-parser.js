    ...
    if (!Array.isArray(value)) {
      ...
      if (indexPropertyRaw) {
        ...
        if (ContextUtil.isPotentialKeyword(indexPropertyRaw)) {
          throw new ErrorCoded(`Keywords can not be used as @index value, got: ${indexPropertyRaw}`, ERROR_CODES.INVALID_TERM_DEFINITION);
        }
        if (typeof indexPropertyRaw !== 'string') {
          throw new ErrorCoded(`@index values must be strings, got: ${indexPropertyRaw}`, ERROR_CODES.INVALID_TERM_DEFINITION);
        }
